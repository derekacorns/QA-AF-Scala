...
@Test
class TestAFVerNoRec {

  def createVerNoRecUserAF(): Unit = {
    ...
    crUserhttprequest2.body("{\n \"user\": {\n \"email\": \"tstvfrnorec9697@acorns.com\",\n \"password\": \"Welcome1\"\n" +
    ...
    crUserhttprequest2.body("{ \"token\":\"" + usrToken + "\",\"user\":{\"profile\":{\"first_name\"" +
      ...
      "\"ssn\":\"676596798\",\"us_resident\":true,\"us_citizen\":true}}}")
    ...
    //Verify User
    var verifUserhttprequest2: RequestSpecification = RestAssured.given().pathParam("uuid", usrUUID).
    ...
    val testBase: VerUserNoRecTestBase = new VerUserNoRecTestBase

    Assert.assertEquals(actualCardValues, testBase.verNoRecExpectedResult())
